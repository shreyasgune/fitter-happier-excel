...
# def foreachmla(xyz,sprintcell,location1,f,m):
#     for x in range(2,xyz.max_row+1):
#         if xyz['E' + str(x)].value == "ATC":
#             muniq = xyz['A' + str(x)].value
#             mlng = xyz['C' + str(x)].value
#             mlat = xyz['B' + str(x)].value
#             mloc =(mlat,mlng)
#             dis1 = distanceCalc(location1,mloc)
#             f.append(dis1)
#             m.append({"mla":dis1,"each":muniq})
#     minmla = f.index(min(f))
#     return m[minmla]

def comparestuff(xyz, sprintcell, location1,rad, maxrow, mla,l, v,mla2):
    for xyrow in range(2, maxrow + 1):
        ...
        v.append({"cellsiteXYZ": xyuniq, "SprintCellsite": sprintcell, "location1": location1, "location2": location2,
                  "distance": dis, "heightdiff": diffh,"xyrad":xyrad,"mla":mla})
    # print min(l)
    # minmla1 = mla1.index(min(mla1))
    # if mla == "ATC":
    #     return v[minmla1]
    minmla2 = mla2.index(min(mla2))
    if mla == "VB":
        return v[minmla2]
...
def doExcelProc(wb):
    ...
    for row in range(2, oc_maxrow+1):
        ...
        minstuff = comparestuff(xyz, uniq, location1,rad, xyz_maxrow,mla, l, v,mla2)
        ...
        res['K' + str(row)].value = minstuff["cellsiteXYZ"]


    # if mla == "ATC":
    #     mlaATC = foreachmla(xyz,uniq,location1,f,m)
    #     res['K'+str(row)].value = mlaATC['each']
